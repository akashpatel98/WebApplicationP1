...
app.config['MYSQL_DATABASE_DB'] = 'deniro'
mysql.init_app(app)
...
def record_view(city_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tbldeiroImport WHERE id=%s', city_id)
    result = cursor.fetchall()
...
def form_edit_get(year_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tbldeniroImport WHERE id=%s', year_id)
    result = cursor.fetchall()
...
def form_update_post(year_id):
    ...
    inputData = (request.form.get('intYear'), request.form.get('intScore'), request.form.get('fldTitle'),
                year_id)
    sql_update_query = """UPDATE tbldeniroImport  SET t.intYear = %s, t.intScore = %s, t.fldTitle = %s """
